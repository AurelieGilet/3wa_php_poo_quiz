...
const customSelectOptions=document.querySelectorAll(".select-option");if(customSelectOptions.length){customSelectOptions.forEach((option)=>{option.addEventListener("click",()=>changeSelectedOption(option,customSelectOptions))})};function handleClickOutsideMenu(event){const{target}=event;if(!target.matches('#options-menu, #nav-options')){const optionsButton=document.getElementById('nav-options');toggleMobileMenu(optionsButton)}}
